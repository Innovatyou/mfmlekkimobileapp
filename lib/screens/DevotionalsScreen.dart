...
class _DevotionalsScreenState extends State<DevotionalsScreen> {
  ...
  Future<void> loadItems() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(res);
        List<Devotionals> _items = parseBranches(res);
  ...
  static List<Devotionals> parseBranches(dynamic res) {
    // final res = jsonDecode(responseBody);
    final parsed = res["devotionals"].cast<Map<String, dynamic>>();
    return parsed
        .map<Devotionals>((json) => Devotionals.fromJson(json))
        .toList();
  }
                    ...
                    children: <Widget>[
                      TableCalendar(
                        ...
                        onDaySelected: (selectedDay, focusedDay) {
                          ...
                          if (_event.length > 0) {
                            Devotionals _events = (_event[0] as Devotionals);
                            Navigator.of(context)
                                .pushNamed(DevotionalViewerScreen.routeName,
...
class ItemTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () {
        Navigator.of(context).pushNamed(DevotionalViewerScreen.routeName,
            arguments: ScreenArguements(
              position: 0,
              items: events,
              itemsList: [],
