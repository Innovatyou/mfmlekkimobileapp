...
import 'package:higherground/models/Versions.dart';
  ...
  initDB() async {
    return await openDatabase(
      ...
      onUpgrade: (Database db, int oldVersion, int newVersion) async {
        ...
        if (oldVersion == 1) {
          /*await db
              .execute("ALTER TABLE ${Notes.TABLE}  ADD preacherid INTEGER");
          await db.execute("ALTER TABLE ${Notes.TABLE}  ADD messageid INTEGER");
          await db.execute("ALTER TABLE ${Notes.TABLE}  ADD preacher TEXT");
          await db
...
//playlists crud
  ...
  saveNote(Notes notes) async {
    final db = await database;
    var result = await db!.rawInsert(
        "INSERT OR REPLACE Into ${Notes.TABLE} (id,title,content,plaincontent,date)"
        " VALUES (?, ?, ?, ?,?)",
        [notes.id, notes.title, notes.content, notes.plaincontent, notes.date]);
    return result;
  }
